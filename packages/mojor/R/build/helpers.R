...
.mojor_rng_c_bridge_state_lines <- function() {
    c(
        "static uint64_t __mojor_rng_state[4] = {0, 0, 0, 0};",
        "static int __mojor_rng_seeded = 0;",
        "static uint64_t __mojor_splitmix64(uint64_t* x) {",
        ...
        "  uint64_t x = seed;",
        "  __mojor_rng_state[0] = __mojor_splitmix64(&x);",
        "  __mojor_rng_state[1] = __mojor_splitmix64(&x);",
        "  __mojor_rng_state[2] = __mojor_splitmix64(&x);",
        "  __mojor_rng_state[3] = __mojor_splitmix64(&x);",
        "  __mojor_rng_seeded = 1;",
        "}",
        "static void __mojor_rng_ensure_seeded(void) {",
        "  if (!__mojor_rng_seeded) {",
