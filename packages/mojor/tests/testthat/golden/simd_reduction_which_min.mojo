    # SIMD arg-reduction for which.min(x) (tie-stable: first index)
    from sys.info import simd_width_of
    comptime simd_width = simd_width_of[DType.float64]()
    if n_i == Int(0):
        idx = Int32(0)
    else:
        var __mojor_best_v: Float64 = x[0]
        ...
        for __mojor_sr_i in range(n_chunks):
            ...
            var __mojor_chunk_best_v: Float64 = vec[0]
            for __mojor_sr_lane in range(1, simd_width):
                var __mojor_lane_v: Float64 = vec[__mojor_sr_lane]
        ...
        for __mojor_sr_rem in range(n_chunks * simd_width, n_i):
            var __mojor_rem_v: Float64 = x[__mojor_sr_rem]
